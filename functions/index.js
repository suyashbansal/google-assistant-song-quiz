            ...
            if (selectedAnswers) {
              ...
              for (let i = 0; i < selectedQuestions.length; i++) {
                if (followUps && followUps.length > 0) {
                  // sessionFollowUps.push('The song was sung by ');
  ...
  const askQuestion = (ssmlResponse, question, answers) => {
    ...
    const askQuestionAudioOnly = () => {
      ...
      if (question) {
        ...
        ssmlResponse.say('I will play the first 15 second of the song and you have to tell me the name of the song');
        ssmlResponse.pause('0.5s');
        ssmlResponse.audio(question, 'song playing', '0s', '15s');
        ssmlResponse.pause('1s');
        ssmlResponse.say('What do you think is the name of the song?');
      }
      // Format the answers
      /* for (let i = 0; i < answers.length; i++) {
